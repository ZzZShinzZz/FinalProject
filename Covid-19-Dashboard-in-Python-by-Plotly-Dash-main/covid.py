# import dash # if dont have, need to pip install dash
import dash
from dash import dcc
from dash import html
from dash.dependencies import Input, Output

# import plotly graph for VISUALIZATION
import plotly.graph_objs as go
import pandas as pd

# USing the Source Code from JohnHop Hospital and 
...
# Unpivot data frames
# Extracting all the same date from all of the Data

confirm_dates = confirmed.columns[4:]
total_confirmed = confirmed.melt(id_vars=['Province/State', 'Country/Region', 'Lat', 'Long'], value_vars=confirm_dates, var_name='date', value_name='confirmed')

death_dates = deaths.columns[4:]
total_deaths = deaths.melt(id_vars=['Province/State', 'Country/Region', 'Lat', 'Long'], value_vars=death_dates, var_name='date', value_name='death')

recover_dates = recovered.columns[4:]
total_recovered = recovered.melt(id_vars=['Province/State', 'Country/Region', 'Lat', 'Long'], value_vars=recover_dates, var_name='date', value_name='recovered')
...
list_locations = covid_data_dict.reset_index().set_index('Country/Region')[['Lat', 'Long']].T.to_dict('dict')
...
app.layout = html.Div([
    html.Div([
        html.Div([
            html.Img(src=app.get_asset_url('coronavirus_logo-2-833x321.jpg'),
                     id='corona-image',
                     style={
                         "height": "100px",
                         "width": "auto",
        ...
        html.Div([
            html.Div([
                html.H3("Covid-19 DashBoard", style={"margin-bottom": "5px", 'color': 'white'}),
                html.H5("Covid-19 Tracker Cases", style={"margin-top": "0px", 'color': 'white'}),
            ])
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.P('new:  ' + f"{covid_data_1['recovered'].iloc[560] - covid_data_1['recovered'].iloc[-2]:,.0f} "
